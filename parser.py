...
def parse_insert(sql: str) -> Dict[str, Any]:
    ...
    if cols and len(cols) != len(values):
        raise ValueError("INSERT: número de columnas y valores no coincide")

    return {
        "op": 2,
        "table": table,
        "columns": cols,        # puede ser None si usaste VALUES sin lista de columnas
        "values": values,
    }
...
def parse_sql_query(text: str) -> Dict[str, Any]:
...
if __name__ == "__main__":
    print(parse_sql_query(r'CREATE TABLE Restaurantes(id INT KEY INDEX SEQ,nombre VARCHAR[20] INDEX BTree,fechaRegistro DATE,ubicacion ARRAY[FLOAT] INDEX RTree)'))
    print(parse_sql_query(r"select * from Restaurantes where ubicacion in ((-12.05,-77.04), 3.5)"))
    print(parse_sql_query(r"select * from Restaurantes where nombre between 'a' and 'm'"))
    print(parse_sql_query(r"insert into Restaurantes values (1,'KFC','2024-01-01',(-12.05,-77.04))"))
    print(parse_sql_query(r"insert into Restaurantes (id,nombre) values (2,'Popeyes')"))
    print(parse_sql_query(r'delete from Restaurantes where id = 5'))
    print(parse_sql_query(r'IMPORT INTO Restaurantes FROM FILE "C:\restaurantes.csv" USING INDEX isam(id)'))
